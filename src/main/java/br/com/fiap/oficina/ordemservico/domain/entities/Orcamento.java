...
public final class Orcamento {
    ...
    public void fechar() {
        if (this.status != StatusOrcamento.ABERTO) {
            throw new DomainException("Orcamento deve estar ABERTO para ser enviado ao cliente. Status atual: " + this.status);
        }
    ...
    public void aprovar() {
        if (this.status != StatusOrcamento.ENVIADO) {
            throw new DomainException("Orcamento deve estar ENVIADO para ser aprovado. Status atual: " + this.status);
        }
    ...
    public void recusar() {
        if (this.status != StatusOrcamento.ENVIADO) {
            throw new DomainException("Orcamento deve estar ENVIADO para ser recusado. Status atual: " + this.status);
        }
        this.status = StatusOrcamento.RECUSADO;
    }

    public void reabrir() {
        if (this.status != StatusOrcamento.ENVIADO && this.status != StatusOrcamento.APROVADO && this.status != StatusOrcamento.RECUSADO) {
